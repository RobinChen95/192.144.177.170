...
class CarAppointSearch extends Controller
{
    ...
    public function excelOutput(){
        $info = model("CarAppointSearch")->getinfo();
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('F1', '驾驶员姓名')
            ->setCellValue('G1', '驾驶员电话')
            ->setCellValue('H1', '车牌号')
        ...
        $spreadsheet->getActiveSheet()->setTitle('临时车辆入校信息');
        $i = 2; //从第二行开始
        foreach ($info as $data) {
            $spreadsheet->getActiveSheet()
                ...
                ->setCellValue('F' . $i, $data['driver_name'])
                ->setCellValue('G' . $i, $data['driver_phone'])
                ->setCellValue('H' . $i, $data['car_number'])
        ...
        header('Content-Disposition: attachment;filename="临时车辆入校信息.xlsx"');
